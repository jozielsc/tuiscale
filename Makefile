...
build:
	@mkdir -p $(BUILD_DIR)
	go build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME) ./cmd/tailscaletui
...
build-static:
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 GOOS=linux go build -ldflags="$(LDFLAGS) -extldflags '-static'" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-amd64 ./cmd/tailscaletui
...
install: build
	...
	@echo "✓ TailscaleTUI instalado em /usr/local/bin/$(BINARY_NAME)"
